...
(() => {
  ...
  class $$Quote_Status_Initial extends Enum {
  constructor() {
    super()
...
class $$Quote_Status_Ok extends Enum {
  constructor() {
    super()
...
class $$Quote_Status_Loading extends Enum {
  constructor() {
    super()
...
class $$Quote_Status_Error extends Enum {
  constructor() {
    super()
...
$$Quote.mappings = {"quote":null,"author":null,"cat":null}

$$Quote.decode = (_input) => {
  let quote = Decoder.field(`quote`, Decoder.string)(_input)
  if (quote instanceof Err) { return quote }

  let author = Decoder.field(`author`, Decoder.string)(_input)
  if (author instanceof Err) { return author }

  let cat = Decoder.field(`cat`, Decoder.string)(_input)
  if (cat instanceof Err) { return cat }

  return new Ok(new $$Quote({
    quote: quote.value,
    author: author.value,
    cat: cat.value
  }))
...
const $QuoteApi = new (class extends Store {
    constructor() {
    super()
    this.state = {
        quote: new $$Quote({
    ...
    status: new $$Quote_Status_Initial()
    }
  }

  get quote() { return this.state.quote }

  get status() { return this.state.status }

  url() {
    return `https://talaikis.com/api/quotes/random/`
  }
  ...
  nextQuote() {
    return (async () => {
      ...
      status: new $$Quote_Status_Loading()
      })), _resolve)
      })

      let response = await $Http.send($Http.get($QuoteApi.url.bind($QuoteApi)()))

      let _2 = $Maybe.toResult(`Json Parse Error`, $Json.parse(response.body))
      ...
      let _3 = $$Quote.decode(body)

      if (_3 instanceof Err) {
        throw _3.value
      }

      let newquote = _3.value

      _result = await new Promise((_resolve) => {
        this.setState(_update(this.state, new Record({
      quote: newquote,
      status: new $$Quote_Status_Ok()
      })), _resolve)
      })
      }
      catch (_error) {
...
$QuoteApi.__displayName = `QuoteApi`

class $Quote extends Component {
  get quote () { return $QuoteApi.quote }

  componentWillUnmount () {
    $QuoteApi._unsubscribe(this)
  }

  componentDidMount () {
    $QuoteApi._subscribe(this)
  }

  render() {
    return _createElement("div", {
      className: `quote-base`
    }, [_createElement("p", {}, [this.quote.quote]), _createElement("p", {
...
class $RefreshButton extends Component {
  get text() {
    return (() => {
      let __condition = this.status
         ...
         if (__condition instanceof $$Quote_Status_Initial) {
          

          return `Get Quote`
        }   if (__condition instanceof $$Quote_Status_Loading) {
          

          return `Loading`
        }   if (__condition instanceof $$Quote_Status_Error) {
  ...
  nextQuote (...params) { return $QuoteApi.nextQuote(...params) }

  get status () { return $QuoteApi.status }
  ...
  render() {
    return _createElement("a", {
      "onClick": (event => (((event) => {
      return this.nextQuote.bind(this)()
      }))(_normalizeEvent(event))),
      className: `refresh-button-base`
    }, [this.text])
  }
}
